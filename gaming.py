...
BOT_TOKEN = "8482210208:AAFv7PUvPv1I-lVX6aTv-YrZaEBwBl6tKKs"
DATA_FILE = "/mnt/data/auction_bot_data.json"
if not os.path.exists(DATA_FILE):
    DATA_FILE = "auction_bot_data.json"

ADMINS = {1766243373, 7343683772, 7952020729, 7460266461, 7879993882, 6639987010, 8006318210}
FUN_ZONE_ID = -1003636955983
FUN_ZONE_USERNAME = "Gamingzone_groupp"
MANAGEMENT_GROUP_ID = -1003571357707
MANAGEMENT_GROUP_LINK = "https://t.me/+fer_gOh6mxwxZWE0"
NOTIFY_GROUP_ID = -1003274871745
NOTIFY_GROUP_LINK = "https://t.me/+hwwgHk_1yOtlMjhl"
...
try:
    ...
    BASE_DIR = os.getcwd()
TEMPLATE_PATH = os.path.join(BASE_DIR, "Hb.png")
AVATAR_FALLBACK = os.path.join(BASE_DIR, "Avatar.png")
...
async def record_start_user(user):
    if not user:
        return False
    key = str(user.id)
    is_new = key not in DATA.get("started_users", {})
    DATA.setdefault("started_users", {})[key] = {
        "id": user.id,
...
async def record_group(chat):
    if not chat:
        return False
    key = str(chat.id)
    is_new = key not in DATA.get("known_groups", {})
    DATA.setdefault("known_groups", {})[key] = {"id": chat.id, "title": chat.title or str(chat.id)}
    save_data(DATA)
    return is_new
...
async def start_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user = update.effective_user
    was_new = await record_start_user(user)
    text = (
        "🏏 Welcome to Gaming Zone Premium League! 🏆\n\n"
"Player registrations are now open!\n"
"Show your cricket skills, compete with the best, and be part of an exciting league.\n\n"
"📌 /Register now & join the action!\n"
"Play. Compete. Win. 💪"
    )
    ...
    if was_new:
        try:
            ...
            await context.bot.send_message(chat_id=NOTIFY_GROUP_ID, text=notify, parse_mode="HTML")
        except Exception:
            pass
...
async def help_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    help_text = (
        "⟦  AUCTION HELP MANUAL  ⟧\n\n"
        "📌 /start_auction  \n"
        "Starts the auction in the group and lets an admin claim the host role.\n\n"
        "📌 /set_table  \n"
        "Host sets the number of teams participating in the auction.\n\n"
        "📌 /team  \n"
        "Host assigns a team name to a user by replying to their message.\n\n"
        "📌 /budget  \n"
        "Host sets starting budget for all teams or a specific team.\n\n"
        "📌 /load  \n"
        "Loads a list of players by replying to a message containing usernames.\n\n"
        "📌 /next  \n"
        "Sends a player to live auction. Works with username, ID, or extra name + username.\n\n"
        "📌 /bid  \n"
        "Team owner (or assistant) places a bid higher than current price.\n\n"
        "📌 /pause  \n"
        "Host pauses the ongoing auction instantly.\n\n"
        "📌 /continue  \n"
        "Host resumes the paused auction.\n\n"
        "📌 /unsold  \n"
        "Shows the list of all players who remained unsold in live auction.\n\n"
        "📌 /my_team  \n"
        "Shows your team’s bought players list and current budget details.\n\n"
        "📌 /status  \n"
        "Shows auction status, team status, or player status (when used with reply/username).\n\n"
        "📌 /assist  \n"
        "Team owner or host assigns an assistant who can bid on behalf of the team.\n\n"
        "📌 /dem  \n"
        "Removes the assistant of a team.\n\n"
        "📌 /access  \n"
        "Host gives full auction access to any user.\n\n"
        "📌 /deduct  \n"
        "Host reduces budget of a team using amount + username/team name.\n\n"
        "📌 /plus  \n"
        "Host increases budget of a team using amount + username/team name.\n\n"
        "📌 /end_auction  \n"
        "Host closes the auction and final team squads are DM-sent to all owners.\n\n"
        "📌 /time  \n"
        "Host sets custom bidding timer between 15–30 seconds.\n\n"
        "⟦  End of Help Manual  ⟧"
    )
...
async def upload_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not msg.reply_to_message:
        await msg.reply_text("Usage: reply to the message you want to upload, then send /upload")
        return
    ...
    for gid_str in list(DATA.get("known_groups", {}).keys()):
        try:
            ...
            continue
        try:
            await context.bot.forward_message(chat_id=gid, from_chat_id=orig.chat.id, message_id=orig.message_id)
        except Exception:
            continue
    try:
        await context.bot.forward_message(chat_id=FUN_ZONE_ID, from_chat_id=orig.chat.id, message_id=orig.message_id)
    except Exception:
        try:
            chat = await context.bot.get_chat(FUN_ZONE_USERNAME)
            if getattr(chat, "id", None):
                try:
                    await context.bot.forward_message(chat_id=chat.id, from_chat_id=orig.chat.id, message_id=orig.message_id)
                except Exception:
                    pass
        except Exception:
            pass
    try:
        await context.bot.forward_message(chat_id=MANAGEMENT_GROUP_ID, from_chat_id=orig.chat.id, message_id=orig.message_id)
    except Exception:
        try:
            stored = DATA.get("management_chat_id")
            if stored:
                try:
                    await context.bot.forward_message(chat_id=int(stored), from_chat_id=orig.chat.id, message_id=orig.message_id)
                except Exception:
                    pass
        except Exception:
            pass
    for uid_str in list(DATA.get("started_users", {}).keys()):
        try:
            ...
            continue
        try:
            await context.bot.forward_message(chat_id=uid, from_chat_id=orig.chat.id, message_id=orig.message_id)
        except Exception:
            continue
    await msg.reply_text("✅ Uploaded and forwarded to known groups & users (attempted).")
...
async def register_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not in_group:
        text = ('❗ Sorry, you must join our main group to register.\n\n' f'👉 <a href="https://t.me/{FUN_ZONE_USERNAME}">Join gaming Zone</a>')
        await update.message.reply_text(text, parse_mode="HTML")
        return
    ...
    try:
        wait_msg = await update.message.reply_text("Wait 10s fetching your profile info...")
    except Exception:
        wait_msg = None

    profile_card_bytes = None
    try:
        ...
        while True:
            ...
            if photos and getattr(photos, "total_count", 0) > 0 and photos.photos:
                ...
                try:
                    ...
                    avatar_stream = None
            if not avatar_stream:
                if os.path.exists(AVATAR_FALLBACK):
                    with open(AVATAR_FALLBACK, "rb") as fh:
                        avatar_stream = BytesIO(fh.read())
                else:
                    avatar_stream = None
            if os.path.exists(TEMPLATE_PATH) and avatar_stream:
                try:
                    ...
                    cx, cy = 953, 402
                    top_left = (int(cx - radius), int(cy - radius))
                    template.paste(avatar, top_left, avatar)
                    out_buf = BytesIO()
                    template.save(out_buf, format="PNG")
                    out_buf.seek(0)
                    profile_card_bytes = out_buf.getvalue()
                    PROFILE_CACHE[reg_id] = profile_card_bytes
                    break
                except Exception as e:
                    logger.exception("image compose attempt failed: %s", e)
                    profile_card_bytes = None
            if asyncio.get_event_loop().time() - start_time > timeout:
                break
            await asyncio.sleep(1.0)
    except Exception as e:
        logger.exception("profile card generation failed overall: %s", e)
        profile_card_bytes = None
    ...
    try:
        if profile_card_bytes:
            bio = BytesIO(profile_card_bytes)
            bio.seek(0)
            if wait_msg:
                try:
                    media = InputMediaPhoto(media=bio, caption=build_text(reg), parse_mode="HTML")
                    await wait_msg.edit_media(media=media, reply_markup=kb_roles)
                except Exception:
                    try:
                        await update.message.reply_photo(photo=bio, caption=build_text(reg), parse_mode="HTML", reply_markup=kb_roles)
                        if wait_msg:
                            try:
            ...
            logger.error("Profile card bytes not created; preview not sent to avoid duplicate text preview.")
    except Exception:
        try:
            await update.message.reply_text(build_text(reg), parse_mode="HTML", reply_markup=kb_roles)
        except Exception:
...
async def role_cb_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        try:
            await cq.edit_message_caption(caption=build_text(reg), parse_mode="HTML", reply_markup=kb_price)
        except Exception:
...
async def price_cb_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        try:
            await cq.edit_message_caption(caption=build_text(reg), parse_mode="HTML", reply_markup=kb_confirm)
        except Exception:
...
async def send_registration_to_management(context: ContextTypes.DEFAULT_TYPE, reg: dict, mg_text: str, mg_kb: InlineKeyboardMarkup) -> int:
    sent = 0
    photo_bytes = PROFILE_CACHE.get(reg["id"])
    try:
        if photo_bytes:
            bio = BytesIO(photo_bytes)
            bio.seek(0)
            try:
                sent_msg = await context.bot.send_photo(chat_id=MANAGEMENT_GROUP_ID, photo=bio, caption=mg_text, parse_mode="HTML", reply_markup=mg_kb)
        ...
        if stored:
            try:
                if photo_bytes:
                    bio = BytesIO(photo_bytes)
                    bio.seek(0)
                    await context.bot.send_photo(chat_id=int(stored), photo=bio)
        ...
        try:
            try:
                if photo_bytes:
                    bio = BytesIO(photo_bytes)
                    bio.seek(0)
                    await context.bot.send_photo(chat_id=gid, photo=bio)
...
async def submit_cb_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "no":
        ...
        try:
            ...
            if getattr(msg_obj, "photo", None):
                try:
                    await cq.edit_message_caption(caption="❌ Registration cancelled.")
                except Exception:
                    try:
    ...
    try:
        ...
        try:
            if getattr(msg_obj, "photo", None):
                await cq.edit_message_caption(caption="✅ Registration submitted to management. Management will review it shortly.")
            else:
...
async def mg_cb_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "mg_accept":
        ...
        try:
            ...
            try:
                await cq.edit_message_caption(caption=edited_caption, parse_mode="HTML")
            except Exception:
                await cq.edit_message_text(edited_caption, parse_mode="HTML")
        except Exception:
            try:
        ...
        try:
            if user_id:
                photo_bytes = PROFILE_CACHE.get(found.get('id'))
                if photo_bytes:
                    with BytesIO(photo_bytes) as ph:
                        ph.seek(0)
                        await context.bot.send_photo(chat_id=int(user_id), photo=ph, caption=accept_text, parse_mode="HTML")
                else:
        ...
        try:
            if user_id:
                ...
                if photo_bytes:
                    with BytesIO(photo_bytes) as ph:
                        ph.seek(0)
                        await context.bot.send_photo(chat_id=int(user_id), photo=ph, caption="❌ Your registration request was declined by management.", parse_mode="HTML")
                else:
...
async def add_role_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    kb_price = InlineKeyboardMarkup([
        [InlineKeyboardButton("1 CR", callback_data=f"add_price|{tmp_id}|1 CR"), InlineKeyboardButton("2 CR", callback_data=f"add_price|{tmp_id}|2 CR"), InlineKeyboardButton("3 CR", callback_data=f"add_price|{tmp_id}|3 CR")]
    ])
    try:
...
async def add_price_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    kb_confirm = InlineKeyboardMarkup([
        [InlineKeyboardButton("✅ Yes — Add", callback_data=f"add_submit|{tmp_id}|yes"), InlineKeyboardButton("❌ Cancel", callback_data=f"add_submit|{tmp_id}|no")]
    ])
    try:
...
async def reset_confirm_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if choice == "no":
        ...
        return
    # --- START: Modified reset behavior: preserve known users & known groups & management chat id ---
    # We'll remove only tournament-related data: tournaments, mg_map registrations/pending, admin temp, pending_remove, reset_tokens, last_broadcast
    keep_started = DATA.get("started_users", {})
    keep_groups = DATA.get("known_groups", {})
    keep_mgmt_chat = DATA.get("management_chat_id", None)
    newdata = {
        "tournaments": {},
        "started_users": keep_started,
        "known_groups": keep_groups,
        "mg_map": {},
        "management_chat_id": keep_mgmt_chat,
        "admin_add_tmp": {},
        "pending_remove": {},
        "reset_tokens": {},
        "last_broadcast": None,
    ...
    save_data(newdata)
    DATA = load_data()
    await cq.edit_message_text("✅ Tournament posts, broadcasts and registration data have been deleted. Known users and groups are preserved.")
        ...
        if added:
            try:
                ...
                notify = (
                    ...
                    f"Group ID   : {chat.id}\n"
                    f"Group Link : {NOTIFY_GROUP_LINK}\n\n"
                    f"Added by   : <a href=\"tg://user?id={adder_id}\">{adder_name}</a>"
                )
                await context.bot.send_message(chat_id=NOTIFY_GROUP_ID, text=notify, parse_mode="HTML")
            except Exception:
                pass
...
def build_app():
    ...
    app.add_handler(CallbackQueryHandler(auction.callback_router, pattern=r"^(auction_host:|auction_table_choice:|auction_table_change:|auction_table_change_confirm:|auction_load:|auction_mode:|end_confirm:|min_buy:|max_buy:|time_confirm:|auto_start_confirm:|auto_start:|auto_launch:)"))
    ...
    app.add_handler(auction.time_handler)
